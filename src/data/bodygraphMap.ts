// BodyGraph Map - EXACT data from robxcodes/gethdchart-web
// https://github.com/robxcodes/gethdchart-web/blob/main/src/utils/map.ts
...
export interface GateMeta {
  position: { x: number; y: number };
  channel: { x: number; y: number; length: number; rotate: number; alwaysRoundCap?: boolean };
}
...
// Centers - EXACT from gethdchart-web
...
// Gates - EXACT positions from gethdchart-web
...
// Helper to get all gates as array
export function getAllGates(): number[] {
  return Object.keys(gates).map(Number);
}

// Helper to check if two gates form a channel
export function isChannel(gate1: number, gate2: number): boolean {
  return channels.some(
    ([g1, g2]) => (g1 === gate1 && g2 === gate2) || (g1 === gate2 && g2 === gate1)
  );
}

// Helper to find the other gate in a channel
export function getChannelPartner(gate: number): number | undefined {
  const channel = channels.find(([g1, g2]) => g1 === gate || g2 === gate);
  if (!channel) return undefined;
  return channel[0] === gate ? channel[1] : channel[0];
}

// Map center IDs from our system to gethdchart system
export function mapCenterId(centerId: string): Centers {
  const idMap: Record<string, Centers> = {
    'ajna': 'mind',
    'solar': 'esp',
    'solarplexus': 'esp',
    'solar_plexus': 'esp',
  ...
  return (idMap[centerId.toLowerCase()] || centerId.toLowerCase()) as Centers;
}
